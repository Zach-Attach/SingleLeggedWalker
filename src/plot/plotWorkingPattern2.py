...
def plotFull(dims):
    ...
    if dims == 1:
        ...
        ax[1].set_yticklabels(['$E_{ft}$', '$E_{f}$', '$E_{b}$'], fontsize=16)
        mainAx.margins(x=0)
        ...
        ax[0][1].imshow(np.roll(newCouplings2,-9,axis=1), cmap=map_RB(), aspect='auto')

    # END: be15d9bcejpp
        ax[1][0].imshow(np.roll(newCouplings2,18,axis=1).transpose(), cmap=map_RB(), aspect='auto')
        # Show all ticks and label them with the respective list entries
        ax[0][1].set_xticks(np.arange(3))
        ax[0][1].set_xticklabels(['$E_{ft}$', '$E_{f}$', '$E_{b}$'], fontsize=16)
        # Show all ticks and label them with the respective list entries
        ax[1][0].set_yticks(np.arange(3))
        ax[1][0].set_yticklabels(['$E_{ft}$', '$E_{f}$', '$E_{b}$'], fontsize=16)
