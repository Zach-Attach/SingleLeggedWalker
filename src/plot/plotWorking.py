...
import numpy as np
from lib.couplingArray import couplingArray, good1N, good2N
...
def plotFull(dims):
    ...
    fig, ax = plt.subplots(2, dims, figsize=((5, 2.5) if dims==1 else (5,5)),  gridspec_kw=grid)
    ...
    newCouplings2 = np.array([couplings[i] for i in good2N])
    ...
    if dims == 1:
        ...
        mainAx = ax[0]
        # Plot the data
        mainAx.scatter(range(len(good1N)), result1, s=40, c='black')
        mainAx.set_yticks(np.array([0.054, 0.058, 0.063]))
        ...
        mainAx.margins(x=0.02)
        # Save the plot to a file
        plt.savefig('plots/scatter1Working.png')
        plt.show()
    else:
        noYTicks(ax[0][1])
        noXTicks(ax[0][0])
        ...
        ax[0][1].set_xticklabels(['$E_{ft}$', '$E_{f}$', '$E_{b}$', '$S_{ft}$', '$S_{a}$'], fontsize=8)
        # Show all ticks and label them with the respective list entries
        ax[1][0].set_yticks(np.arange(5))
        ax[1][0].set_yticklabels(['$E_{ft}$', '$E_{f}$', '$E_{b}$', '$S_{ft}$', '$S_{a}$'], fontsize=8)
        ax[1, 1].set_visible(False)

        mainAx.margins(x=0)
        # Save the plot to a file
        plt.savefig('plots/scatter2Working.png')
        plt.show()
