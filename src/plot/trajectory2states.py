...
ax4.plot([i/10 for i in range(len(n1))], n1, color='blue')
ax4.plot([i/10 for i in range(len(n2))], n2, color='red')

ax4.set_xlabel('Time')
ax4.set_ylabel('Neuron State', fontsize=8)  # Set the font size to 8

num = '224_18_Output' # 170
...
# Plotting the second plot with switched axes and horizontal lines
# plt.figure(1, dpi=800, figsize=(5,5))
ax3.plot([i/10 for i in range(len(n1))], n1, color='blue')
ax3.plot([i/10 for i in range(len(n2))], n2, color='red')
...
ax2.plot([i/10 for i in range(len(all_data[:600]))], all_data[:600])
...
ax1.plot([i/10 for i in range(len(all_data[:600]))], all_data[:600])
